...
_exe_path = r'./tesseract-ocr-w64-setup-v5.2.0.20220712.exe'
...
pytesseract.pytesseract.tesseract_cmd = _exe_path
...
def read_from_image(file_name, language):
    ...
    # if the image is not skewed, the function returns the same image
    try:
        deskewed = correct_skew(no_shadows)
        deskewed_copy = deskewed.copy()
    except TesseractError:
        deskewed = no_shadows
        deskewed_copy = no_shadows.copy()

    # applying gaussian blur to reduce noize
    ...
    ocr = pytesseract.image_to_data(
        otsu, output_type=Output.DICT, config=custom_oem_psm_config, lang=language
    )

    draw_image_boxes = True

    boxes = len(ocr["text"])
    texts = []
    for i in range(boxes):
        if int(float(ocr["conf"][i])) > 64:
            if draw_image_boxes:
                (x, y, w, h) = (
                    ocr["left"][i],
                    ocr["top"][i],
    ...
    result = list_to_string(texts)
    return result


start_time = time.time()

file_name = "test/images/ID-UK.png"
assert os.path.exists(file_name)

result = read_from_image(file_name, "jpn")
print("String: ", result)
